...
![Zencastr画面](./images/starting-to-podcasts/zencastr_01.png){width=80%}

![Zencastrゲスト画面](./images/starting-to-podcasts/zencastr_02.png){width=80%}
...
概要としてはZoomなどに代表されるWebミーティングツールで会話しつつ、そのツールの音声と自分のマイク音声をそれぞれOBS Studioというツールで録音（録画）する仕組みです。手順は次の通りです。

#### OBS Studioのインストール
https://obsproject.com/ja/download

ホームページからダウンロードしてインストールするのみです。

#### OBS Studioの設定
設定画面の出力から、出力モードを詳細にした上で設定します。設定するのは録画の音声トラック部分を複数有効しにます（画像では1と3を有効）

![OBS Studioの音声設定](./images/starting-to-podcasts/OBS_01.png)

次に音声ミキサーの設定を開きます。バージョンによって開き方が変わる可能性がありますが、メイン画面の音声ミキサーの歯車アイコンから起動できます。

![OBS Studioの音声設定](./images/starting-to-podcasts/OBS_01.png)

図のようにさきほど設定したトラックをそれぞれ指定し、デスクトップ音声（ミーティングツールからの音声）とマイク（自分の音声）を別トラックで録音できるようにします。

この状態で録画すると音声トラックが分かれた状態で録画できます。Podcastとしては映像はいらないので映像ソースを指定する必要はないのですが、もしビデオPodcastをやるときは逆に映像も録画することができます。

#### Audacityにffmpegライブラリをインストールする
録画したファイルはこの設定だとmkvという拡張子で出力され、音声はAACとして内蔵されています。AudacityはデフォルトでAACを取り込めないので、ffmpegライブラリをインストールします。

https://lame.buanzo.org/ffmpeg.php

URLからWindows用インストーラーをダウンロードして実行します。その後、Audacityを起動して編集→環境設定→ライブラリを開き、ffmpegライブラリが自動検出されることを確認します。検出されない場合はファイルダイアログからインストールしたdllファイルを指定しましょう。

#### Audacityにmkvファイルをドラッグ&ドロップする
あとはmkvファイルをAudacityにドラッグ&ドロップするだけで、図のように音声トラックが複数検出されます。

![OBS Studioの音声設定](./images/starting-to-podcasts/mkv_import_01.png)

トラック分ドロップして指定を繰り返すと音声トラックを分けることができました。

#### 3人以上の場合
この方法だと1対1なら別トラックにできますが、ミーティングツールに複数人いる場合は別トラックにできません。その場合はまた別の工夫が必要となります。いまのところ、簡単な解決策は思いついていないですが、なにか良い案があればまた次回の改訂の際に紹介させてください。
...
私を含めた数人で、都内某所に秘密基地を作りました。この成立の経緯などは別の記事<span class="footnote">錬金術ラボへのお誘い おやかた note https://note.mu/oyakata2438/n/n61dfd82ab189</span>に譲りますが、本書の著者のひとりでもある、KANE@higuyumeさんがPodcast収録場所として使うようになり、活動メンバーも増え、ボドゲ会やモブプロ会など、活動の幅が一気に広がりました。場があることで活動が広がるという面もありますが、単純にPodcastの収録場所として考えても、比較的静かで、周りを気にしなくてよいという点、お酒のストックがあるのでその気になれば飲みながらできること、複数人や深夜でも収録できることなど、さまざまなメリットがあります。Podcasterのみなさん、Podcastに出てみたいなーというかた、一度遊びに来てみませんか？詳細は、著者の誰かに声をかけてください。
</div>
